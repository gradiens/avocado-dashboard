...
/*Login Controller***************************************/
/********************************************************/
appControllers.controller('loginCtrl', ['$scope', 'loginService', 'fbURL', function ($scope, loginService, fbURL) {
...
appControllers.controller('gradMainCtrl', ['$scope', 'loginService', 'gradMainService', 'helperService', 'fbURL', function ($scope, loginService, gradMainService, helperService, fbURL) {
	...
	//Morris*************************
	 ...
	 $scope.$watch(function () {
            return gradMainService.totals;
        },
        function (newVal, oldVal) {
            console.log("inside watch", newVal);
            $scope.morrOptions = newVal;
		    console.log("Reconstructing graphs");
		    setCharts();
	...
	function setCharts(){
		_.each($scope.morrOptions, function(el, ind){
			_.each(el, function(element, index){
				try{
					if(!graphs[index])
						graphs[index] = Morris.Donut(element);
					else
						graphs[index].setData(element.data);
				}
				catch(err){
					console.log("Small error while loading Morris", err);
				}
			});
		});
